 ...
 */
export async function enqueueWeeklyDigest(userId) {
    await reportQueue.add("weekly-digest", { userId }, { jobId: `digest-${userId}-${Date.now()}` });
}
/**
 * Enqueue weekly digests for ALL active users.
 * This is called by the report worker's repeatable schedule.
 */
export async function enqueueAllWeeklyDigests() {
    // This will be called from the report worker's cron job
    await reportQueue.add("trigger-all-digests", {}, { jobId: `trigger-digests-${Date.now()}` });
