...
class ReviewService {
    ...
    async processReview(reviewId) {
        try {
            ...
            if (review.status !== "PENDING") {
                console.log(`Review ${reviewId} is already ${review.status}, skipping`);
                return;
            }
            ...
            if (!user?.accessToken) {
                throw new Error(`User ${review.repository.userId} has no access token`);
            }
            // 4. Fetch the commit diff from GitHub
            const diff = await githubService.getCommitDiff(user.accessToken, review.repository.fullName, review.commitId);
            ...
            const aiResult = await aiService.reviewCode(diff, review.repository.fullName);
            // 6. Store the issues
            ...
            console.log(`Review ${reviewId} completed: score=${aiResult.score}, issues=${aiResult.issues.length}`);
     ...
     */
    async getReviewsByRepository(repositoryId) {
