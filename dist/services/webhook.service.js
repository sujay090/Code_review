...
class WebhookService {
     ...
     */
    async registerWebhook(accessToken, repoFullName) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to register webhook on ${repoFullName} (${response.status}): ${errorBody}`);
        }
     ...
     */
    async removeWebhook(accessToken, repoFullName, webhookId) {
        ...
        if (!response.ok && response.status !== 404) {
            throw new Error(`Failed to remove webhook ${webhookId} from ${repoFullName} (${response.status})`);
        }
    }
    /**
