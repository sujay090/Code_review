 ...
 */
export const createManualReview = async (req, res, next) => {
    try {
        if (!req.user) {
            res.status(401).json({ message: "Not authenticated" });
            return;
        ...
        const latestCommit = await githubService.getLatestCommit(user.accessToken, repository.fullName, branch);
        ...
        console.log(`Manual review created: ${review.id} for commit ${latestCommit.sha.slice(0, 7)} on ${repository.fullName}`);
        res.status(202).json({
