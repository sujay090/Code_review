...
const reportWorker = new Worker("report-generation", async (job) => {
    ...
    if (job.name === "trigger-all-digests") {
        ...
        console.log(`[ReportWorker] Found ${users.length} users with active repos`);
        // Enqueue individual digest jobs
        for (const user of users) {
            await reportQueue.add("weekly-digest", { userId: user.id }, { jobId: `digest-${user.id}-${Date.now()}` });
        }
        return;
    }
    ...
    if (job.name === "weekly-digest") {
        ...
        console.log(`[ReportWorker] Generating weekly digest for user ${userId}`);
        const user = await conn.user.findUnique({
            where: { id: userId },
        });
        if (!user || !user.email) {
            console.warn(`[ReportWorker] User ${userId} not found or has no email`);
            return;
        }
        ...
        if (reviews.length === 0) {
            console.log(`[ReportWorker] No reviews for user ${userId} this week, skipping`);
            return;
        }
        // Calculate stats
        ...
        const totalIssues = reviews.reduce((sum, r) => sum + r._count.issues, 0);
        // Group by repository
        ...
        console.log(`[ReportWorker] Weekly digest sent to ${user.email} — ${totalReviews} reviews, avg ${avgScore}`);
    }
}, {
    connection,
    concurrency: 2,
...
reportQueue
    ...
    .then(() => {
    console.log("[ReportWorker] Weekly digest cron scheduled (Monday 9:00 AM)");
})
    .catch((err) => {
