...
const emailWorker = new Worker("email-notifications", async (job) => {
    ...
    console.log(`[EmailWorker] Sending notification for review ${reviewId} (attempt ${job.attemptsMade + 1})`);
    ...
    if (review.status !== "COMPLETED") {
        console.warn(`[EmailWorker] Review ${reviewId} is ${review.status}, skipping email`);
        return;
    }
    const user = review.repository.user;
    if (!user.email) {
        console.warn(`[EmailWorker] User ${user.id} has no email, skipping`);
        return;
    }
    ...
    console.log(`[EmailWorker] Email sent for review ${reviewId} → ${user.email} (${result})`);
}, {
    connection,
    concurrency: 5,
});
